      ...
      <section className="relative w-full overflow-hidden bg-nv-gradient rounded-b-[24px] min-h-[480px]">
        ...
        <div className="relative max-w-6xl mx-auto px-6 md:px-10 pt-10 pb-10 grid md:grid-cols-2 gap-6 items-end">
          <div className="relative z-10">
            <Breadcrumbs items={[{ label: t('nav.home'), href: '/' }, { label: t('about.title') }]} variant="light" />
            <h1 className="font-display font-bold text-nv-dark text-[40px] md:text-[56px] leading-tight mt-3">
              {t('about.title')}
            </h1>
            <p className="mt-4 max-w-md text-nv-text leading-7">{t('about.intro')}</p>

            <h3 className="font-display font-bold text-nv-dark text-xl mt-8">{t('about.followUs')}</h3>
          ...
          <div className="relative h-[360px] md:h-[460px] -mb-16 overflow-visible flex items-end justify-center">
            <Image src="/main.png" alt="Nevlemar" width={800} height={1200} className="object-contain object-bottom w-full h-full" />
          </div>
        </div>
      </section>

      <section className="max-w-3xl mx-auto px-6 md:px-10 py-16 text-center">
        <h2 className="font-display font-bold text-nv-dark text-3xl">{t('about.contactTitle')}</h2>
        <p className="text-nv-text mt-3">{t('about.contactText')}</p>
        <div className="mt-8 space-y-3 text-nv-dark">
          <div>
            <span className="font-bold">{t('about.phone')}: </span>
            <a className="underline" href="tel:+380501756050">+38 (050) 175-6050</a>
          </div>
          <div>
            <span className="font-bold">{t('about.email')}: </span>
            <a className="underline" href="mailto:nevlemar@gmail.com">nevlemar@gmail.com</a>
          </div>
        </div>
      </section>
    </div>
