...
async function fetchDogs(locale?: string): Promise<Dog[]> {
  try {
    const r = await api.post('/dogs/find', { breed: 'pom' }, { params: { locale } });
    return r.data || [];
  } catch {
    return [];
  }
...
export default async function PomsPage({ params: { locale } }: { params: { locale: string } }) {
  const t = await getTranslations();
  const [dogs, site] = await Promise.all([fetchDogs(locale), getSiteContent(locale)]);

  const card = site.breedsSection?.cards?.find((c) => c.slug === 'pomeranian') || site.breedsSection?.cards?.[0];
  const title = card?.title || t('breeds.pomeranian');
