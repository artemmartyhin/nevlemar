...
async function fetchDogs(): Promise<Dog[]> {
  try {
    const r = await api.post('/dogs/find', { breed: 'pom' });
    return r.data || [];
  } catch {
    return [];
  }
...
export default async function PomsPage() {
  const t = await getTranslations();
  const dogs = await fetchDogs();
  return (
    <div>
      <BreedHero
        title={t('breeds.pomeranian')}
        subtitle={t('breeds.lookAtOurDogs')}
        crumbs={[{ label: t('nav.home'), href: '/' }, { label: t('breeds.pomeranian') }]}
      />
